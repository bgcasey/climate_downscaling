...
missing<-rbind(ABMI2022_cleaned, ABMI2023_cleaned)%>%
  select(mission_id, latitude, longitude, accuracy, retrieval_date, deployment_date)%>%
  distinct()

save(missing, file="2_pipeline/tmp/missing_ABMI.rData")
write_csv(missing, file="2_pipeline/tmp/missing_ABMI_metadata.csv")
