...
data<-data_full%>%
  ...
  dplyr::select(Tmax_diff, Tavg_diff, Tmin_diff, CHILI, HLI, 
  ...
  dplyr::select(-c(Tmax_diff, Tavg_diff))
...
tuned_param_min <- hyper_grid %>%
  dplyr::arrange(min_RMSE) 
save(tuned_param_min, file="2_pipeline/store/tuned_param_min.rData")


# 2. Boosted Regression Tree ----
load("2_pipeline/store/tuned_param_min.rData")


## 2.1. Apply `dismo::gbm.step` to tuned parameters ----
min_brt_1 <- gbm.step(data = random_data, 
                  gbm.x = 2:ncol(random_data), 
                  gbm.y = 1, 
                  family = "gaussian", 
                  tree.complexity = tuned_param_min$interaction.depth[1],
                  n.minobsinnode = tuned_param_min$n.minobsinnode[1],
                  learning.rate = tuned_param_min$shrinkage[1],
                  bag.fraction = tuned_param_min$bag.fraction[1],
                  silent = FALSE,
                  plot.main = FALSE)
save(min_brt_1, file = "3_output/models/terrain/min_brt_1.rData")

## 2.2. Get model stats ----
min_brt_1_stats <- calculate_brt_stats(model = min_brt_1)
save(min_brt_1_stats, file = "3_output/model_results/min_brt_1_stats.rData")

# 3. Simplified Boosted Regression Tree ----
## 3.1. Drop variables that don't improve model performance ----


min_brt_1_simp <- gbm.simplify(min_brt_1)
save(min_brt_1_simp, file = "2_pipeline/store/models/min_brt_1_simp.rData")

## 3.2. Remove non-numeric characters from the row names ----
rownames(min_brt_1_simp$deviance.summary) <- 
  gsub("[^0-9]", "", rownames(min_brt_1_simp$deviance.summary))

## 3.3. Get the optimal number of drops ----
optimal_no_drops <- as.numeric(rownames(
  min_brt_1_simp$deviance.summary %>% slice_min(mean)))

## 3.4. Remove dropped variables from the dataframe ----
random_data <- random_data %>%
  dplyr::select(Tmin_diff, min_brt_1_simp$pred.list[[optimal_no_drops]])

## 3.5 BRT Second Iteration ----
min_brt_2 <- gbm.step(data = random_data, 
                  gbm.x = 2:ncol(random_data), 
                  gbm.y = 1, 
                  family = "gaussian", 
                  tree.complexity = tuned_param_min$interaction.depth[1],
...
save(min_brt_2, file = "2_pipeline/store/models/min_brt_2.rData")

## 3.6 Get model stats ----
summary(min_brt_2)
min_brt_2_stats <- calculate_brt_stats(model = min_brt_2)
save(min_brt_2_stats, file = "3_output/model_results/min_brt_2_stats.rData")
...
bootstrap_brt <- function(data, n_iterations, n_cores) {
  ...
  covariate_columns <- as.data.frame(replicate(ncol(data)-1, 
                                               numeric(), 
                                               simplify = FALSE))
  ...
  run_iteration <- function(i) {
    tryCatch({
      ...
      samp <- sample(nrow(data), round(0.75 * nrow(data)), replace = TRUE)
      ...
      # Add model name and evaluation metrics to model_stats
  ...
  results <- mclapply(1:n_iterations, run_iteration, mc.cores = n_cores)
...
bootstrap_models <- bootstrap_brt(random_data, n_iterations, n_cores)

# Save models
save(bootstrap_models, file = "3_output/model_results/bootstrap_models_min.rData")
