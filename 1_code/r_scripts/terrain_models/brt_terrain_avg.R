...
tuned_param_avg <- hyper_grid %>%
  dplyr::arrange(min_RMSE) 
save(tuned_param_avg, file="2_pipeline/store/tuned_param_avg.rData")
...
avg_brt_1 <- gbm.step(data = random_data, 
                  gbm.x = 2:ncol(random_data), 
                  gbm.y = 1, 
                  family = "gaussian", 
                  tree.complexity = tuned_param_avg$interaction.depth[1],
                  n.minobsinnode = tuned_param_avg$n.minobsinnode[1],
                  learning.rate = tuned_param_avg$shrinkage[1],
                  bag.fraction = tuned_param_avg$bag.fraction[1],
                  silent = FALSE,
                  plot.main = FALSE)
save(avg_brt_1, file = "3_output/models/terrain/avg_brt_1.rData")

## 2.2. Get model stats ----
avg_brt_1_stats <- calculate_brt_stats(model = avg_brt_1)
save(avg_brt_1_stats, file = "3_output/model_results/avg_brt_1_stats.rData")

# 3. Simplified Boosted Regression Tree ----
## 3.1. Drop variables that don't improve model performance ----


avg_brt_1_simp <- gbm.simplify(avg_brt_1)
save(avg_brt_1_simp, file = "2_pipeline/store/models/avg_brt_1_simp.rData")

## 3.2. Remove non-numeric characters from the row names ----
rownames(avg_brt_1_simp$deviance.summary) <- 
  gsub("[^0-9]", "", rownames(avg_brt_1_simp$deviance.summary))

## 3.3. Get the optimal number of drops ----
optimal_no_drops <- as.numeric(rownames(
  avg_brt_1_simp$deviance.summary %>% slice_min(mean)))

## 3.4. Remove dropped variables from the dataframe ----
random_data <- random_data %>%
  dplyr::select(Tavg_diff, avg_brt_1_simp$pred.list[[optimal_no_drops]])

## 3.5 BRT Second Iteration ----
avg_brt_2 <- gbm.step(data = random_data, 
                  gbm.x = 2:ncol(random_data), 
                  gbm.y = 1, 
                  family = "gaussian", 
                  tree.complexity = 3,  
...
save(avg_brt_2, file = "2_pipeline/store/models/avg_brt_2.rData")

## 3.6 Get model stats ----
summary(avg_brt_2)
avg_brt_2_stats <- calculate_brt_stats(model = avg_brt_2)
save(avg_brt_2_stats, file = "3_output/model_results/avg_brt_2_stats.rData")
...
# Save models
save(bootstrap_models, file = "3_output/model_results/bootstrap_models_avg.rData")
