...
d1<-d %>%
    group_by(Site_StationKey,Month) %>%
    ...
    ungroup()%>%
    filter(between(Tmax_dif, quantile(Tmax_dif, .25)-(3*IQR(Tmax_dif)), quantile(Tmax_dif, .75)+(3*IQR(Tmax_dif))))%>%
    filter(between(Tmin_dif, quantile(Tmin_dif, .25)-(3*IQR(Tmin_dif)), quantile(Tmin_dif, .75)+(3*IQR(Tmin_dif))))%>%
    filter(between(Tavg_dif, quantile(Tavg_dif, .25)-(3*IQR(Tavg_dif)), quantile(Tavg_dif, .75)+(3*IQR(Tavg_dif))))



# check if outliers vary by season. (may indicate that they are caused by snow burial
da<-d1%>%
  filter(Tmax_dif< -10)
...
d<-ibuttons_ERA5_2%>%
  ...
  select(-consecutive_day_flag, lag_date)%>%
  # arrange(Site_StationKey, Date)%>%
